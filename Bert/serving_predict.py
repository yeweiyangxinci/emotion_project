...
class TextProcessor(DataProcessor):
  def _create_example(self, data_dir):
    examples = []
    ...
    for line in lines:
      ...
      examples.append(InputExample(
        guid="unused_id", text_a=line_data[1], text_b=None, label=line_data[0]
      ))
    return examples

  def get_labels(self):
    return ['科普', '科技', '国际', '正能量', '社会', '数码', '健康', '辟谣',
                '体育', '运动健身', '情感', '政务', '财经', '股市', '电影', '电视剧',
                '明星', '军事', '历史', '游戏', '综艺', '旅游', '校园', '法律', '房产',
                '家居', '动漫', '宗教', '三农']

  def get_train_examples(self, data_dir):
    return self._create_example(os.path.join(data_dir, "train.tsv"))

  def get_dev_examples(self, data_dir):
    return self._create_example(os.path.join(data_dir, "dev.tsv"))

  def get_test_examples(self, data_dir):
    return self._create_example(os.path.join(data_dir, "test.tsv"))
...
class PolarityProcessor(DataProcessor):
  def _create_example(self, data_dir):
      ...
      for line in lines:
          ...
          line_data = line.split("\t")
          examples.append(InputExample(
              guid="unused_id", text_a=line_data[0], text_b=None, label=line_data[1]
          ))
      return examples
  ...
  def get_train_examples(self, data_dir):
    return self._create_example(os.path.join(data_dir, "train.txt"))

  def get_dev_examples(self, data_dir):
    return self._create_example(os.path.join(data_dir, "test.txt"))
...
def run_classifier(task_name, init_checkpoint, prediction_arr):
    ...
    basic_dir = "/DataDisk_1/shiwandong_project/model_project/sentiment_emotion/Bert/"
...
class BertClassifier:
    ...
    def run_server(self, task_name, line):
        ...
        # convert_single_example是由函数InputExample转来，主要是将序列token化（如果序列是语句），
        # 然后将得到的token序列转换成vocab.txt列表中对应的id序列, 即 input_ids, 相当于id表示字词+token buff
        # 另外mask_ids表示是否为PaddingInputExample, segment_ids表示是句1还是句2,
        # 我的任务是用100条信号预测下一条信号, 于是label_id是下一条信号ID
        feature = convert_single_example(0, example, label_list, max_seq_length, tokenizer)
        input_ids = np.reshape([feature.input_ids], (1, max_seq_length))
        input_mask = np.reshape([feature.input_mask], (1, max_seq_length))
        segment_ids = np.reshape([feature.segment_ids], (max_seq_length))
    ...
    def new_server(self, text_arr, task_name, id_arr = None):
        basic_dir = r"/root/xiandao_project/emotion_project/sentiment_emotion/Bert/"
        vocab_file = basic_dir + "chinese_L-12_H-768_A-12/vocab.txt"
        do_lower_case = True
        max_seq_length = 512
        batch_size = 10
        ...
        len_arr = len(text_arr)
        rounds = len_arr // batch_size
        remainder = len_arr % batch_size

        response_arr = []

        for idx in range(0, rounds+1):
            input_ids_arr = []
            input_mask_arr = []
            segment_ids_arr = []
            label_ids_arr = []
            ...
            if idx < rounds:
                for line_idx in range(idx*batch_size, (idx+1)*batch_size):
                    text_a = text_arr[line_idx]
                    if task_name == 'triple':
                        example = InputExample(guid=0, text_a=text_a, label="愤怒")
                    elif task_name == 'text':
                        example = InputExample(guid=0, text_a=text_a, label="科普")
                    else:
                        example = InputExample(guid=0, text_a=text_a, label="1")
                    feature = convert_single_example(0, example, label_list, max_seq_length, tokenizer)
                    input_ids_arr.append(feature.input_ids)
                    input_mask_arr.append(feature.input_mask)
                    segment_ids_arr.append(feature.segment_ids)
                    label_ids_arr.append(feature.label_id)
                #print("1已经处理数据",(idx+1)*batch_size,"条")
                #print("Now：", datetime.datetime.now())

            else:
                for line_idx in range(idx*batch_size, len_arr):
                    text_a = text_arr[line_idx]
                    if task_name == 'triple':
                        example = InputExample(guid=0, text_a=text_a, label="愤怒")
                    elif task_name == 'text':
                    ...
                    segment_ids_arr.append(feature.segment_ids)
                    label_ids_arr.append(feature.label_id)
                #print("1已经处理数据", len_arr, "条")
                #print("Now：", datetime.datetime.now())
                batch_size = remainder

            input_ids = np.reshape([input_ids_arr], (batch_size, max_seq_length))
            input_mask = np.reshape([input_mask_arr], (batch_size, max_seq_length))
            segment_ids = np.reshape([segment_ids_arr], (batch_size, max_seq_length))
            label_ids = [label_ids_arr]
            dict_curl = {"input_ids": input_ids.tolist(),
                         "input_mask": input_mask.tolist(),
                         "segment_ids": segment_ids.tolist(),
                         "label_ids": label_ids}
            ...
            dict_data = {"inputs": dict_curl, "signature_name": "vectorize_service"}
            if task_name == 'triple':
                url = 'http://localhost:8501/v1/models/triple_model:predict'
            elif task_name == 'text':
                url = 'http://localhost:8501/v1/models/text_model:predict'
            else:
                url = 'http://localhost:8501/v1/models/polarity_model:predict'
            requests.adapters.DEFAULT_RETRIES = 5
            resp = requests.post(url, json=dict_data)
            #print(resp.json())
            response_arr.append(resp.json()['outputs'])
            s = requests.session()
            s.keep_alive = False
        backinfo = []
        for pro_arr in response_arr:
            for probabilities in pro_arr:
        ...
        #         max_idx = probabilities.index(max_val)
        #         line_label = label_list[max_idx]
        #         obj = {"id":id_arr[id_idx], "label":line_label, "proba":max_val}
        #         id_idx += 1
        #         backinfo.append(obj)

        return backinfo

    def eva_model(self, text_arr, task_name, GPU=None):
        basic_dir = "/DataDisk_1/shiwandong_project/model_project/sentiment_emotion/Bert/"
        vocab_file = basic_dir + "chinese_L-12_H-768_A-12/vocab.txt"
        do_lower_case = True
        max_seq_length = 512
        ...
        for idx in range(0, rounds + 1):
            ...
            if task_name == "polarity":
                if GPU == "gpu_one":
                    url = "http://10.0.0.161:8503/v1/models/Bert_polarity_two:predict"
                else:
                    url = "http://10.0.0.161:8503/v1/models/Bert_polarity:predict"
            else:
                url = "http://10.0.0.161:8504/v1/models/Bert_text_two:predict"
            requests.adapters.DEFAULT_RETRIES = 5
            resp = requests.post(url, json=dict_data)
            # print(resp.json())
            response_arr.append(resp.json()['outputs'])
